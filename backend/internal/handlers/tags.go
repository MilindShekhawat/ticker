...
func ListTicketTags(c *fiber.Ctx) error {
	...
	tags, err := models.GetTagsByTicketID(ticketID)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{
			"error": err.Error(),
		})
...
func AddTagToTicket(c *fiber.Ctx) error {
	ticketID, err := c.ParamsInt("id")
	if err != nil {
		return c.Status(400).JSON(fiber.Map{
			"error": "Invalid ticket ID",
	...
	if err := models.AddTagToTicket(ticketID, req.TagID); err != nil {
		if errors.Is(err, models.ErrTagNotFound) {
			return c.Status(404).JSON(fiber.Map{
				"error": "Tag not found",
			})
...
func RemoveTagFromTicket(c *fiber.Ctx) error {
	ticketID, err := c.ParamsInt("id")
	if err != nil {
		return c.Status(400).JSON(fiber.Map{
			"error": "Invalid ticket ID",
	...
	if err := models.RemoveTagFromTicket(ticketID, tagID); err != nil {
		return c.Status(500).JSON(fiber.Map{
			"error": err.Error(),
		})
	}
